  ...
  depthai_ros_msgs__msg__TrackedFeatures__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    depthai_ros_msgs__msg__TrackedFeatures * data =
      (depthai_ros_msgs__msg__TrackedFeatures *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!depthai_ros_msgs__msg__TrackedFeatures__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          depthai_ros_msgs__msg__TrackedFeatures__fini(&output->data[i]);
        }
        return false;
      }
    }
